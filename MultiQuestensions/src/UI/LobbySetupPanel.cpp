...
namespace MultiQuestensions::UI {
	...
	void LobbySetupPanel::AddSetupPanel(UnityEngine::RectTransform* parent, GlobalNamespace::MultiplayerSessionManager* sessionManager) {
		...
		horizontal->set_padding(UnityEngine::RectOffset::New_ctor(0, 0, 18, 0));
		...
		customSongsToggle = QuestUI::BeatSaberUI::CreateToggle(vertical4->get_transform(), "Custom Songs", getConfig().config["customsongs"].GetBool(), SetCustomSongs);
