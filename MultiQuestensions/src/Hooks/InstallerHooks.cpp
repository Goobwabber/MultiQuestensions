...
namespace MultiQuestensions {
    MAKE_HOOK_MATCH(MultiplayerConnectedPlayerInstaller_InstallBindings, &MultiplayerConnectedPlayerInstaller::InstallBindings, void, MultiplayerConnectedPlayerInstaller* self) {
        ...
        self->dyn__sceneSetupData()->gameplayModifiers->dyn__zenMode() = getConfig().config["LagReducer"].GetBool();
        getLogger().debug("[Prefix] ZenMode is set to '%s'", self->dyn__sceneSetupData()->gameplayModifiers->dyn__zenMode() ? "true" : "false");
        MultiplayerConnectedPlayerInstaller_InstallBindings(self);
        getLogger().debug("[Postfix] ZenMode is set to '%s'", self->dyn__sceneSetupData()->gameplayModifiers->dyn__zenMode() ? "true" : "false");
    }
